...
I store this file in an S3 (Simple Storage Service) bucket with a DynamoDB (Amazon's NoSQL key-value database) table handling the locking, so it is safe and consistent. This setup lives in `terraform-bootstrap` and runs once per AWS account before anything else. It is the only thing that has to be done before the main infrastructure.

**Step 2 - Networking**

I create a VPC (Virtual Private Cloud), which is a private, isolated network inside AWS that all my resources will live in. Along with the VPC, I create subnets (smaller network segments inside the VPC), route tables (rules that control where network traffic goes), and an S3 VPC Endpoint. The S3 endpoint is important because it lets services inside the VPC talk to S3 without going out to the public internet, which is both faster and cheaper.

Nothing else can be created without a network to put it in, so this always comes first.

**Step 3 - Data lake storage**

I create five S3 buckets: Bronze, Silver, Gold, Quarantine, and a fifth one for Athena query results. These are just empty buckets at this point. No data is in them yet. But they need to exist before any of the services that write to them can be set up.

**Step 4 - Permissions and metadata**

I create a KMS (Key Management Service) encryption key that will encrypt all data at rest across the platform. I also create IAM (Identity and Access Management) roles for every service that needs one: Glue, Airflow, Redshift, DMS (Database Migration Service), and the ECS (Elastic Container Service) task for the AI agent. Each role only has permission to do exactly what that service needs and nothing more.
...
I create the RDS (Relational Database Service) PostgreSQL database that acts as my source system. This is the database that the CDC (Change Data Capture) process will read changes from.

I also create the DMS (Database Migration Service) replication instance and replication task. DMS connects to PostgreSQL, reads its WAL (Write-Ahead Log, the internal journal PostgreSQL keeps of every change), and converts those changes into Parquet files that land in the Bronze S3 bucket. This is where data ingestion begins.
...
I create the MWAA (Amazon Managed Workflows for Apache Airflow) environment, which is the managed Airflow service on AWS. Airflow is what runs the pipeline on a schedule and controls the order that things run in. I also create the ECS (Elastic Container Service) cluster and task definition for the AI Operations Agent.

One note on MWAA: it costs about $0.49 per hour, which adds up quickly. I only spin this up when I need to test orchestration or record a demo. For day-to-day development of DAGs (Directed Acyclic Graphs, which are Airflow's way of defining a pipeline), I run Airflow locally.
...
I write dbt (data build tool) models, which are SQL files with some extra structure. These read from Silver through Athena and produce business-level aggregations that go into Gold. Things like daily revenue by product, monthly active users, weekly order volumes.
...
I write the DAGs (Directed Acyclic Graphs) that orchestrate the whole sequence. A DAG is a Python file that defines a set of tasks and the order they run in. My main pipeline DAG looks roughly like this:
...
```
DMS (Database Migration Service) replication lag spikes
...
When an event fires, the agent does not just react to that single event. It calls the AWS SDK (Software Development Kit, the library used to interact with AWS services from code) across multiple services at the same time to get the full picture before taking any action.
...
Every incident produces a plain-English report sent via SNS (Simple Notification Service, which delivers messages to email, Slack, or SMS):
...
Data is stored as Parquet files (a column-oriented file format that is much faster to query than CSV), partitioned by date with one folder per source table.
...
Athena (Amazon's serverless SQL query engine) writes query result files here whenever it runs a SQL query. This is just a designated output location for the Athena workgroup.
...
Step 2:  AWS DMS (Database Migration Service) reads that change from PostgreSQL's
         internal WAL (Write-Ahead Log) and writes it as a Parquet file to Bronze S3
...
Step 8:  BI (Business Intelligence) tools connect to Redshift and analysts build dashboards
...
| AWS DMS (Database Migration Service) | Managed database migration service | Captures CDC events and writes to Bronze |
...
| dbt (data build tool) | SQL transformation framework | Runs SQL models for Silver to Gold |
...
| Redshift Spectrum | Redshift feature | Queries Gold S3 data as external tables without loading it |
| Amazon MWAA (Managed Workflows for Apache Airflow) | Managed Airflow service | Orchestrates and schedules the pipeline |
| Apache Airflow | Workflow orchestration engine | DAGs (Directed Acyclic Graphs) define task order |
| AWS KMS (Key Management Service) | Encryption key management | Encrypts all data at rest |
| AWS IAM (Identity and Access Management) | Permission control | Controls what each service is allowed to do |
...
| ECS Fargate (Elastic Container Service) | Serverless container runtime | Runs the AI Operations Agent |
| Claude (Anthropic) | Large language model | Powers the agent's cross-service reasoning |
| VPC (Virtual Private Cloud) | Private AWS network | Isolates all compute from the public internet |
...
Non-overlapping ranges mean I can peer these networks together in the future if needed. A /16 range gives 65,536 available IP addresses. A /20 range gives 4,096 per subnet.
...
- AWS CLI (Command Line Interface) v2: `aws --version` should show 2.x
- Terraform 1.6 or higher: `terraform --version`
- AWS SSO (Single Sign-On) profiles configured for dev-admin, staging-admin, and prod-admin
- Python 3.9 or higher: `python --version`
- dbt CLI: `pip install dbt-athena-community`
- Anthropic API (Application Programming Interface) key for the AI agent, stored as the `ANTHROPIC_API_KEY` environment variable
...
PostgreSQL keeps an internal journal called the WAL (Write-Ahead Log) that records every insert, update, and delete before it is applied. AWS DMS (Database Migration Service) reads this journal and converts those entries into files. This is much cheaper than dumping the whole database repeatedly and it captures changes in near real time.

### What is a DAG (Directed Acyclic Graph)?

A DAG is how Airflow represents a pipeline. The name "Directed Acyclic Graph" means a set of tasks where each task depends on the ones before it, and there are no circular dependencies (a task cannot wait for itself to finish).
...
I store this file in S3 (remote state) so it is shared across sessions and protected. A DynamoDB (Amazon's NoSQL database) table prevents two Terraform runs from happening at the same time and corrupting the file.
...
|---|---|
| KMS (Key Management Service) encryption at rest | All S3 buckets, RDS, DMS |
| Dedicated IAM (Identity and Access Management) roles | Glue, MWAA, Redshift, DMS, and ECS each have their own role |
...
I keep costs low by destroying dev at the end of each session. The two most expensive always-on services are MWAA at about $0.49 per hour and RDS (Relational Database Service). When I am not actively working, those are gone.
...
| API Key | Authenticated with an API (Application Programming Interface) key | None | Pay per token |
